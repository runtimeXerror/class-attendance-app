...
export default function TeacherAttendanceDashboard() {
  ...
  const avg = data.students.length
    ...
    : 0;
  const safeCount = data.students.filter(s => s.percentage >= 75).length;
  const warnCount = data.students.filter(s => s.percentage < 75 && s.percentage >= 50).length;
  const critCount = data.students.filter(s => s.percentage < 50).length;
    ...
    <View style={{ flex: 1, backgroundColor: theme.bg }}>
      ...
      >
        ...
        <View style={styles.statusRow}>
          ...
          <View style={[styles.statusPill, { backgroundColor: theme.warningLight }]}>
            <Text style={[styles.statusVal, { color: theme.warning }]}>{warnCount}</Text>
            <Text style={[styles.statusLbl, { color: theme.warning }]}>⚠ Warning</Text>
          </View>
          <View style={[styles.statusPill, { backgroundColor: theme.dangerLight }]}>
            ...
            <Text style={[styles.statusLbl, { color: theme.danger }]}>✗ Critical</Text>
          </View>
        </View>
