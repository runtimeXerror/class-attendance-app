      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 40 }}>
        ...
        <View style={[styles.header, { backgroundColor: theme.primary }]}>
          ...
          <View style={styles.subjChipRow}>
            ...
            {data.subject.credits ? (
              <View style={styles.subjChip}>
                <Text style={styles.subjChipTxt}>
                  Credit • {Number(data.subject.credits).toFixed(1)}
                </Text>
              </View>
            ) : null}
          </View>
        ...
        <View style={[styles.tabs, { backgroundColor: theme.card, borderColor: theme.border }]}>
          {tabs.map((t) => (
            <TouchableOpacity
              key={t.key}
              style={[styles.tab, activeTab === t.key && { backgroundColor: theme.primary }]}
        ...
        {activeTab === "summary" && (
          <>
            <View style={[styles.bigPctCard, { backgroundColor: pctColor.bg }]}>
              <Text style={[styles.bigPctLabel, { color: theme.textSecondary }]}>Attendance</Text>
            ...
            <View style={[styles.chartCard, { backgroundColor: theme.card, borderColor: theme.border }]}>
              <Text style={[styles.chartTitle, { color: theme.text }]}>Attendance Breakdown</Text>
              ...
              <AnimatedBar
                ...
                color={theme.success} trackColor={theme.successLight} textColor={theme.text}
              />
              <AnimatedBar
                label="Absent" value={absent} total={total}
                color={theme.danger} trackColor={theme.dangerLight} textColor={theme.text}
              />
            </View>
          </>
        )}
        ...
        {activeTab === "analysis" && (
          <View>
            ...
            {/* ── Donut Pie Chart ── */}
            <View style={[styles.chartCard, { backgroundColor: theme.card, borderColor: theme.border }]}>
              <Text style={[styles.chartTitle, { color: theme.text }]}>Distribution</Text>
              <Text style={{ fontSize: 11, color: theme.textMuted, marginBottom: 10, fontStyle: "italic" }}>
                Tap green (present) or red (absent) to lift & see details
              </Text>
              <View style={{ alignItems: "center", marginVertical: 8 }}>
                <AttendancePieChart
                  ...
                  absent={absent}
                  size={220}
                  presentColor={theme.success}
                  absentColor={theme.danger}
            ...
            <View style={[styles.analysisCard, {
              backgroundColor:
                data.analysis.status === "Safe" ? theme.successLight
                : data.analysis.status === "Warning" ? theme.warningLight
                : theme.dangerLight,
            }]}>
              <Text style={[styles.analysisLabel, { color: theme.textSecondary }]}>Status</Text>
              <Text style={[styles.analysisValue, {
                color:
                  data.analysis.status === "Safe" ? theme.success
                  : data.analysis.status === "Warning" ? theme.warning
                  : theme.danger,
              }]}>
                {data.analysis.status === "Safe" ? "✓ Safe"
                  : data.analysis.status === "Warning" ? "⚠ Warning"
                  : "⛔ Critical"}
              </Text>
...
const styles = StyleSheet.create({
  ...
  subjChip: {
    backgroundColor: "rgba(255,255,255,0.22)", paddingHorizontal: 12, paddingVertical: 6,
  ...
  tabs: {
    flexDirection: "row", padding: 4, borderRadius: Radius.md,
  ...
  chartCard: { padding: 16, borderRadius: Radius.lg, borderWidth: 1, marginBottom: 12 },
